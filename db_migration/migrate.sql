...
INSERT INTO events_eventdate (id, eventdate_name, eventdate_date, event_id)
    SELECT id, eventdate_name, eventdate_date, event_id FROM old_public.events_eventdate;
SELECT setval('events_eventdate_id_seq', COALESCE((SELECT MAX(id)+1 FROM events_eventdate), 1), false);
...
--
-- Fixing references to django_content_type (content_types framework)
-- This is needed because content types are autogenerated by Django
-- and id numbers do not match.
--

UPDATE django_comments
    SET content_type_id = (
        SELECT public.django_content_type.id
            FROM public.django_content_type
            INNER JOIN old_public.django_content_type
                ON old_public.django_content_type.model = public.django_content_type.model AND
                   old_public.django_content_type.app_label = public.django_content_type.app_label
            WHERE old_public.django_content_type.id = django_comments.content_type_id);
    ...
    SET content_type_id = (
        SELECT public.django_content_type.id
            ...
            INNER JOIN old_public.django_content_type
                ON old_public.django_content_type.model = public.django_content_type.model AND
                   old_public.django_content_type.app_label = public.django_content_type.app_label
            WHERE old_public.django_content_type.id = django_admin_log.content_type_id);
    ...
    SET content_type_id = (
        SELECT public.django_content_type.id
            ...
            INNER JOIN old_public.django_content_type
                ON old_public.django_content_type.model = public.django_content_type.model AND
                   old_public.django_content_type.app_label = public.django_content_type.app_label
            WHERE old_public.django_content_type.id = reversion_version.content_type_id);

UPDATE tagging_taggeditem
    SET content_type_id = (
        SELECT public.django_content_type.id
            FROM public.django_content_type
            INNER JOIN old_public.django_content_type
                ON old_public.django_content_type.model = public.django_content_type.model AND
                   old_public.django_content_type.app_label = public.django_content_type.app_label
            WHERE old_public.django_content_type.id = tagging_taggeditem.content_type_id);
