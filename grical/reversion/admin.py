...
from grical.reversion.revisions import DEFAULT_SERIALIZATION_FORMAT
...
class VersionAdmin(admin.ModelAdmin):
    ...
    def _autoregister(self, model, follow=None):
        ...
        if not grical.reversion.is_registered(model):
            follow = follow or []
            ...
            grical.reversion.register(model, follow=follow, format=self.reversion_format)
    
    def __init__(self, *args, **kwargs):
        """Initializes the VersionAdmin"""
        super(VersionAdmin, self).__init__(*args, **kwargs)
        # Automatically register models if required.
        if not grical.reversion.is_registered(self.model):
            inline_fields = []
    ...
    def log_addition(self, request, object):
        ...
        grical.reversion.revision.user = request.user
        grical.reversion.revision.comment = _(u"Initial version.")
        grical.reversion.revision.ignore_duplicates = self.ignore_duplicate_revisions
        
    def log_change(self, request, object, message):
        """Sets the version meta information."""
        super(VersionAdmin, self).log_change(request, object, message)
        grical.reversion.revision.user = request.user
        grical.reversion.revision.comment = message
        grical.reversion.revision.ignore_duplicates = self.ignore_duplicate_revisions
    ...
    def log_deletion(self, request, object, object_repr):
        ...
        grical.reversion.revision.user = request.user
        grical.reversion.revision.comment = _(u"Deleted %(verbose_name)s." % {"verbose_name": self.model._meta.verbose_name})
        grical.reversion.revision.ignore_duplicates = self.ignore_duplicate_revisions
