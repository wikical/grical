...
import grical
from grical.reversion.models import Version, Revision, VERSION_ADD, VERSION_CHANGE, VERSION_DELETE
from grical.reversion.revisions import RegistrationError, DEFAULT_SERIALIZATION_FORMAT
...
class ReversionRegistrationTest(TestCase):
    ...
    def setUp(self):
        ...
        grical.reversion.register(TestModel)
        
    def testCanRegisterModel(self):
        """Tests that a model can be registered."""
        self.assertTrue(grical.reversion.is_registered(TestModel))
        # Check that duplicate registration is disallowed.
        self.assertRaises(RegistrationError, lambda: grical.reversion.register(TestModel))
        
    def testCanReadRegistrationInfo(self):
        """Tests that the registration info for a model is obtainable."""
        registration_info = grical.reversion.revision.get_registration_info(TestModel)
    ...
    def testCanUnregisterModel(self):
        ...
        grical.reversion.unregister(TestModel)
        self.assertFalse(grical.reversion.is_registered(TestModel))
        # Check that duplicate unregistration is disallowed.
        self.assertRaises(RegistrationError, lambda: grical.reversion.unregister(TestModel))
...
class ReversionCreateTest(TestCase):
    ...
    def testRevisionContextManager(self):
        ...
        with grical.reversion.revision:
            test = TestModel.objects.create(name="test1.0")
    ...
    def testRevisionDecorator(self):
        ...
        @grical.reversion.revision.create_on_success
        def create_revision():
...
class ReversionCustomRegistrationTest(TestCase):
    ...
    def setUp(self):
        ...
        TestModel.objects.all().delete()
        # Register the model.
        grical.reversion.register(TestModel, fields=("id",), format="xml")
        # Create some initial revisions.
        with grical.reversion.revision:
            self.test = TestModel.objects.create(name="test1.0")
        with grical.reversion.revision:
    ...
    def testCustomRegistrationHonored(self):
        ...
        self.assertEqual(grical.reversion.revision.get_registration_info(TestModel).fields, ("id",))
        self.assertEqual(grical.reversion.revision.get_registration_info(TestModel).format, "xml")
    ...
    def testIgnoreDuplicates(self):
        ...
        with grical.reversion.revision:
            grical.reversion.revision.ignore_duplicates = True
            self.test.save()
        self.assertEqual(len(Version.objects.get_for_object(self.test)), 4)
...
class ReversionRelatedTest(TestCase):
    ...
    def setUp(self):
        ...
        # Register the models.
        grical.reversion.register(TestModel, follow=("testrelatedmodel_set",))
        grical.reversion.register(TestRelatedModel, follow=("relation",))
    ...
    def tearDown(self):
        ...
        grical.reversion.unregister(TestModel)
        grical.reversion.unregister(TestRelatedModel)
        # Clear the database.
        Version.objects.all().delete()
        TestModel.objects.all().delete()
        TestRelatedModel.objects.all().delete()
...
class ReversionManyToManyTest(TestCase):
    ...
    def setUp(self):
        ...
        # Register the models.
        grical.reversion.register(TestModel, follow=("testmanytomanymodel_set",))
        grical.reversion.register(TestManyToManyModel, follow=("relations",))
    ...
    def tearDown(self):
        ...
        # Unregister the models.
        grical.reversion.unregister(TestModel)
        grical.reversion.unregister(TestManyToManyModel)
        # Clear the database.
        Version.objects.all().delete()
        TestModel.objects.all().delete()
        TestManyToManyModel.objects.all().delete()
...
try:
    from grical.reversion.helpers import generate_patch, generate_patch_html
except ImportError:
    pass
else:
